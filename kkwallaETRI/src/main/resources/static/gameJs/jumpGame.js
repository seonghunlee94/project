...
$(document).ready(function() {
	...
	$("#santaMsg").html("산타: 누르는 동안 장대가 늘어나요<br>저를 다음 블럭으로 이동시켜주세요.");
	let filteredTable;
	let randomNum;
	let santaMsg;
  ...
  const audioFalling = document.getElementById("audioFalling");
  const mainAudio = document.getElementById("jumpBGM");
		...
		switch (status) {
			...
			case "turning": {
				...
				if (sticks.last().rotation > 90) {
					...
					$.getJSON("../gameDB/JumpGameDB.json", function(data) {
						filteredTable = data.filter(function(element) {
							return element.status == "walking";
						});
						randomNum = Math.floor(Math.random() * filteredTable.length);
						santaMsg = filteredTable[randomNum].sentence;
						$("#santaMsg").html(santaMsg);
					});
					status = "walking";
				}
				break;
